...
export default function PumpManagement({ language = 'ar' }: PumpManagementProps) {
  ...
  const calculatePumpSales = (pump: any) => {
    const prev = (prevReadings[pump.id] ?? pump.previousReading ?? 0);
    const curr = (readings[pump.id] ?? pump.currentReading ?? 0);
    const litersSold = curr - prev;
  ...
  const handleSaveReading = (pumpId: string, newReading: number) => {
    ...
    const prev = (prevReadings[pumpId] ?? pump.previousReading ?? 0);
    const curr = newReading;
    const liters = curr - prev;
    if (liters > 0) {
      const amount = liters * (pump.sellPrice || 0);
      recordSale(pumpId, liters, amount);
    }

    // Move the entered reading to previousReading and currentReading
    updatePump(pumpId, {
      previousReading: curr,
      currentReading: curr,
    });
    
    // Clear inputs so placeholders show the saved value in previous reading
    setReadings(prevState => {
      const next = { ...prevState };
      delete next[pumpId];
      return next;
    });
    setPrevReadings(prevState => {
      const next = { ...prevState };
      delete next[pumpId];
      return next;
    });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pumps.filter(pump => pump.isActive).map((pump) => {
            ...
            const hasNewReading = readings[pump.id] !== undefined;
              ...
              <Card key={pump.id} className="bg-white shadow-sm hover:shadow-md transition-shadow">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {/* Previous Reading Input - styled consistently */}
                    <div className="space-y-1">
                      <Label className="text-sm font-medium">{t.previousReading}</Label>
                      <div className="relative">
                        <Input
                          type="number"
                          value={prevReadings[pump.id] ?? ''}
                          onChange={(e) => setPrevReadings(prev => ({
                            ...prev,
                            [pump.id]: parseFloat(e.target.value) || 0
                          }))}
                          placeholder={(pump.previousReading || 0).toString()}
                          className={`font-mono text-lg mt-1 bg-slate-50 ${isRTL ? 'text-right pr-12' : 'pl-3 pr-12'}`}
                        />
                        <span className={`pointer-events-none absolute top-1/2 -translate-y-1/2 ${isRTL ? 'left-3' : 'right-3'} text-xs text-slate-500`}>لتر</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        <Input
                          ...
                          placeholder={(prevReadings[pump.id] ?? pump.previousReading ?? 0).toString()}
                          className={`font-mono text-lg mt-1 bg-slate-50 ${isRTL ? 'text-right pr-12' : 'pl-3 pr-12'}`}
                        />
                        <span className={`pointer-events-none absolute top-1/2 -translate-y-1/2 ${isRTL ? 'left-3' : 'right-3'} text-xs text-slate-500`}>لتر</span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-mono text-lg font-bold text-slate-900">
                            {(((readings[pump.id] ?? 0) - (prevReadings[pump.id] ?? pump.previousReading ?? 0)).toFixed(2))} لتر
                          </p>
                        </div>
                        <div>
                          <Label className="text-slate-700">
                          ...
                          <p className="font-mono text-lg font-bold text-slate-900">
                            {((((readings[pump.id] ?? 0) - (prevReadings[pump.id] ?? pump.previousReading ?? 0)) * (pump.sellPrice || 0)).toFixed(2))} {t.currency}
                          </p>
                        </div>
                      </div>
                    </div>
        ...
        <Card className="bg-white">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              ملخص المضخات اليومية
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {pumps.filter(p => p.isActive).map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border bg-slate-50">
                  <span className="font-medium text-slate-900">{p.name}</span>
                  <div className="flex items-center gap-6">
                    <div className="text-sm">
                      <span className="text-slate-600 ml-2">الترات المباعة</span>
                      <span className="font-mono font-bold">{(p.dailySales?.liters || 0).toFixed(2)} لتر</span>
                    </div>
                    <div className="text-sm">
                      <span className="text-slate-600 ml-2">الإيرادات</span>
                      <span className="font-mono font-bold">{(p.dailySales?.amount || 0).toFixed(2)} {t.currency}</span>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className="border-t pt-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-6">
                  <span className="font-mono font-bold text-blue-600">
                    {pumps.filter(p => p.isActive).reduce((sum, p) => sum + (p.dailySales?.liters || 0), 0).toFixed(2)} لتر
                  </span>
                  <span className="font-mono font-bold text-green-600">
                    {pumps.filter(p => p.isActive).reduce((sum, p) => sum + (p.dailySales?.amount || 0), 0).toFixed(2)} {t.currency}
                  </span>
                </div>
              </div>
            </div>
